      ...
      return (
        ...
        >
          ...
          >
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {items
                    ...
                    .map((item: any, idx: number) => (
                      ...
                      >
                        <div className="h-12 w-20 shrink-0 overflow-hidden rounded-xl">
                          <img
                            src={item.coverUrl}
                            alt={item.title}
            ...
            {/* Book Items Preview - Always visible */}
            {hasBooks && (
              <div className="mt-6 border-t border-white/70 pt-5">
                <div className="mb-3 flex items-center gap-2 text-gray-500">
                ...
                <div className="flex flex-wrap gap-2">
                  {items.filter((item: any) => item.type !== 'course').slice(0, 3).map((item: any, idx: number) => (
                    <Link
                      key={`${order.id}-book-preview-${idx}`}
                      href={`/book/${item.bookId}`}
                      className="group flex items-center gap-2 rounded-xl border border-white/70 bg-gray-50/80 px-2 py-2 transition-all hover:bg-orange-50/45"
                    >
                      <div className="h-20 w-12 shrink-0 overflow-hidden rounded shadow-sm">
                        {item.coverUrl ? (
                      ...
                      <p className="text-xs font-bold text-gray-700 truncate max-w-[120px]">{item.title}</p>
                      {item.quantity > 1 && <span className="text-xs text-gray-500">x{item.quantity}</span>}
                    </Link>
                  ))}
                  {items.filter((item: any) => item.type !== 'course').length > 3 && (
                    <div className="flex items-center rounded-xl bg-gray-100 px-2 py-2 text-xs font-bold text-gray-500">
                      +{items.filter((item: any) => item.type !== 'course').length - 3} more
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="mt-4 flex justify-end border-t border-white/70 pt-3">
              <div
                role="button"
                tabIndex={0}
                onClick={(e) => {
                  ...
                  <div className="space-y-3">
                    {items.map((item: any, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          <div className="h-20 w-12 shrink-0 overflow-hidden rounded-xl shadow-sm transition-transform group-hover:scale-[1.03]">
                            {item.coverUrl ? (
                              <img
                                src={item.coverUrl}
                                alt={item.title}
