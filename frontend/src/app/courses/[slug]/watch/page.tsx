...
export default function CourseWatchPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const saveProgress = (nc: Set<number>, nls: Record<number,number>, nfs: Record<number,number>, nw: Set<number>, nvp: Record<number,number>) => {
    if (!course || !user) return;
    localStorage.setItem(`course_progress_${user.uid}_${course.id}`, JSON.stringify({
      completedLessons: Array.from(nc), lessonQuizScores: nls,
      finalQuizScores: nfs, watchedLessons: Array.from(nw), videoProgress: nvp,
    }));
  };
  ...
  useEffect(() => {
    if (course && user) {
      const saved = localStorage.getItem(`course_progress_${user.uid}_${course.id}`);
      if (saved) {
        const p = JSON.parse(saved);
        setCompletedLessons(new Set(p.completedLessons || []));
        setLessonQuizScores(p.lessonQuizScores || {});
        setFinalQuizScores(p.finalQuizScores || {});
        setWatchedLessons(new Set(p.watchedLessons || []));
        setVideoProgress(p.videoProgress || {});
        const s100: Record<number,boolean> = {};
        Object.entries({ ...(p.lessonQuizScores || {}), ...(p.finalQuizScores || {}) }).forEach(([id, sc]) => {
          if (sc === 100) s100[parseInt(id)] = true;
        });
      ...
      <div className="rounded-3xl border overflow-hidden bg-gradient-to-br from-gray-800/60 to-gray-900/60 border-gray-700/40">
        <div className="px-6 py-8 text-center space-y-4">
          ...
          <button onClick={onReset}
            className="mt-1 inline-flex items-center gap-2 px-5 py-2.5 bg-gray-700/60 hover:bg-gray-600/60 border border-gray-600/40 text-white rounded-xl text-sm font-medium transition-all">
