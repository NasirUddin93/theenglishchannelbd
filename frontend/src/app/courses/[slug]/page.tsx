...
interface CourseDetailResponse extends Course {
  sections?: CourseSection[]; quizzes?: CourseQuiz[]; access_time?: string; language?: string;
}
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => {
        ...
        return (
          <span key={s} className={`relative inline-block ${interactive ? 'cursor-pointer hover:scale-125' : ''} transition-transform`} onClick={interactive ? () => onRate?.(s) : undefined}>
            <Star className={`${sz} text-gray-200 fill-gray-200`} />
            {(filled || partial) && (
              <span
...
export default function CourseDetailPage() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      // For courses, any non-cancelled order counts as enrollment
      for (const o of local) {
        if (!o?.items) continue;
        if (o.userId && String(o.userId) !== String(user.uid)) continue;
        if (o.status === 'cancelled') continue;
        if (o.items.some((i: any) => (i?.type === 'course' || i?.courseId) && String(i.courseId || i.bookId) === String(course.id))) { 
          enrolled = true; 
          break; 
        }
      }
      ...
      if (!enrolled) {
        try {
          ...
          for (const o of orders) {
            if (o.status === 'cancelled') continue;
            if ((o.items || []).some((i: any) => String(i.course_id || i.courseId) === String(course.id))) { 
              enrolled = true; 
              break; 
            }
          }
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      ...
      const newReviews = [rv, ...reviews.filter(r => r.id !== rv.id)];
      setReviews(newReviews);
      setAverageRating(newReviews.filter(r => r.is_approved).reduce((sum, r) => sum + r.rating, 0) / Math.max(1, newReviews.filter(r => r.is_approved).length));
  ...
  const handleQuestionSubmit = async () => {
    if (!course || !newQuestion) return;
    const name = user ? user.displayName : guestQuestionName.trim();
    if (!name) { alert('Please enter your name.'); return; }
    setSubmitting(true);
    try {
      const res = await fetch(`/api/courses/${course.slug}/questions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_name: name, user_email: user ? user.email : guestQuestionEmail.trim() || undefined, question: newQuestion }) });
      if (!res.ok) throw new Error();
      const { question: q } = await res.json();
      setQuestions(prev => [q, ...prev]); setNewQuestion(''); setGuestQuestionName(''); setGuestQuestionEmail('');
          ...
          <div className="flex flex-col lg:flex-row gap-10 items-center">
            ...
            <div className="flex-1 space-y-5">
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                {[
                  { icon: <Clock className="w-4 h-4" />, label: `${totalDuration > 0 ? `${Math.floor(totalDuration / 60)}h ${totalDuration % 60}m` : `${course.duration_hours}h`} total` },
                  { icon: <BookOpen className="w-4 h-4" />, label: `${course.lessons_count} lessons` },
                  { icon: <Globe className="w-4 h-4" />, label: `${course.access_time || 'Lifetime'} access` },
                ].map(s => (
                ...
                <div className="p-7">
                  ...
                  {activeTab === 'overview' && (
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      {[
                        { icon: <Clock className="w-5 h-5 text-orange-500" />,    label: 'Duration',   value: totalDuration > 0 ? `${Math.floor(totalDuration / 60)}h ${totalDuration % 60}m` : `${course.duration_hours}h` },
                        ...
                        { icon: <Infinity className="w-5 h-5 text-orange-500" />, label: 'Access',     value: course.access_time || 'Lifetime' },
                      ...
                      {reviews.length > 0 && (
                        <div className="flex items-center gap-6 p-5 rounded-2xl bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-100">
                          <div className="text-center">
                            ...
                            <div className="mt-2"><Stars rating={averageRating} /></div>
                          ...
                          {!user && (
                            <div className="grid grid-cols-2 gap-3">
                              <input
                                ...
                                className="w-full p-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition-all placeholder-gray-400"
                              />
                              <input
                                type="email"
                                value={guestQuestionEmail}
                                onChange={e => setGuestQuestionEmail(e.target.value)}
              ...
              <div className="sticky top-20 space-y-4">
                ...
                <div className="bg-white rounded-3xl border border-gray-100 shadow-xl shadow-gray-100/60 overflow-hidden">
                  ...
                  <div className="px-7 py-5 space-y-4">
                    {[
                      { icon: <Clock className="w-4 h-4 text-orange-400" />,        label: 'Duration',   value: totalDuration > 0 ? `${Math.floor(totalDuration / 60)}h ${totalDuration % 60}m` : `${course.duration_hours}h` },
                      ...
                      { icon: <Globe className="w-4 h-4 text-orange-400" />,         label: 'Language',   value: course.language || 'English' },
                      { icon: <Infinity className="w-4 h-4 text-orange-400" />,      label: 'Access',     value: course.access_time || 'Lifetime' },
